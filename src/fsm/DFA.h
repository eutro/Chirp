...
namespace fsm {
  ...
  public:
     ...
     */
    bool accept(Idx &current, S &symbol) const {
    ...
    DFA<S, F> minimise() const {
      ...
      do {
        ...
        for (const S &symbol : alphabet) {
          for (Idx i = 0; i < states.size() - 1; ++i) {
            ...
            Idx iTransition = iFound != states[i].transitions.end() ? iFound->second : states.size();
            for (Idx j = i + 1; j < states.size(); ++j) {
              if (distinguishable[i][j]) continue;

              auto jFound = states[j].transitions.find(symbol);
              Idx jTransition = jFound != states[j].transitions.end() ? jFound->second : states.size();
      ...
      for (Idx i = 0; i < states.size(); ++i) {
        ...
        {
          Idx state = grouped[i] = optimised.push();
    ...
    void compileInto(const std::string &sTy, const std::string &fTy, std::ostream &os) const {
      ...
      os << "bool CompiledDFA::accept(Idx &current, " << sTy << " &symbol) const {\n";
      os << "  switch (current) {\n";
      for (Idx state = 0; state < states.size(); ++state) {
        os << "    case " << state << ":";
        ...
        os << "      switch (symbol) {\n";
        std::map<Idx, std::vector<const S *>> trev;
      ...
      os << "  return false;\n";
      os << "}\n";
      os << "" << fTy << " CompiledDFA::finished(Idx state) const {\n";
      os << "  switch (state) {\n";
      for (Idx state = 0; state < states.size(); ++state) {
        os << "    case " << state << ":";
        const DFAState<S, F> &stateV = states.at(state);
        os << " return (" << fTy << ")0x" << std::hex << (Idx) stateV.finished << std::dec << ";\n";
      }
      os << "  }\n";
      os << "  return (" << fTy << ")0;\n";
      os << "}\n";
